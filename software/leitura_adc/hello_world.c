...
int main()
{
  ...
  alt_up_adc_auto_disable(adc); //Desabilita a atualização automática do ADC
  printf("Hello from Nios II!\n");
  delay(100);
  while(1){
	    alt_up_adc_update(adc); //Faz a atualização dos registradores do ADC
	  	delay(100); //Delay para permitir uma melhor vizualização, o
	  	// Lógica para mostrar o resultado dos 12 bits no display
